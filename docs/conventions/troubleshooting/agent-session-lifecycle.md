...
- Root cause:
  ...
  message payloads.
- Fix:
  Persist reported counts on assistant `payload_json.usage`. If tokens arrive
  first, backfill the later assistant row and do not restamp a later turn
  with the same snapshot. Keep `session_metadata_json.usage.tokens` /
  `lastTurn` as a latest-turn fallback. Do not invent missing keys. Archive
  writers in the sessionarchive consumer still must copy that stored object
  onto each usage-bearing archive event.
- Validation:
  `go test ./packages/agent/store-sqlite -run 'TokenUsage|SessionRuntimeContextPreservesLastTurn|ReportSessionStatePersistsLastTurn|ReportActivityStatePersistsCodex|ReportSessionMessagesPersistsAssistant|ReportSessionMessagesBackfills'`
  and
