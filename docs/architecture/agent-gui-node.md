...
disclosure spacing. Native text selection is not a geometry input. Virtualized
turns share a layout origin, so a Range that crosses turns paints as a
page-sized highlight in WKWebView. Virtual items use `user-select: none` except
message bodies (`[data-agent-transcript-native-select]`); copy across messages
uses the message Copy control. Live Ranges are not forcibly clipped mid-drag.
Escape, a pointer down outside the message body, a host
