...
test("queued send-next retains its target until capabilities arrive", () => {
  const harness = createHarness(true);
  ...
  harness.engine.dispatch({
    ...
    agentSessionId: "session-1",
    awaitingTurnExpiresAtUnixMs: 30_100,
    cancelCommandId: "cancel-queued-guidance",
    promptId: "queued-guidance",
    timeoutMs: 30_000
  });
  ...
  assert.equal(
    harness.engine.getSnapshot().promptQueue.recordsBySessionId["session-1"]
      ?.pendingSendNowByPromptId?.["queued-guidance"]?.targetTurnId,
    "turn-1"
  );
  ...
  assert.equal(command?.type === "queue/sendPrompt" && command.guidance, true);
  assert.equal(
    command?.type === "queue/sendPrompt" ? command.targetTurnId : null,
    "turn-1"
  );
});
