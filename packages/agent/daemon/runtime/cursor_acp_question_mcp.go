...
func parseCursorAskQuestionRequest(raw json.RawMessage) (cursorAskQuestionRequest, map[string]any, error) {
	...
	for index, question := range request.Questions {
		...
		if question.ID == "" || question.Prompt == "" {
			return cursorAskQuestionRequest{}, nil, fmt.Errorf("cursor ask_question question %d requires id and prompt", index+1)
		}
		if question.AllowMultiple {
			return cursorAskQuestionRequest{}, nil, fmt.Errorf("cursor ask_question question %q uses unsupported multi-select", question.ID)
		}
		...
		questions = append(questions, map[string]any{
			"id": question.ID, "header": question.Prompt, "question": question.Prompt,
			"prompt": question.Prompt, "options": options, "multiSelect": false,
			"allowMultiple": false, "allowFreeText": false,
		})
	}
