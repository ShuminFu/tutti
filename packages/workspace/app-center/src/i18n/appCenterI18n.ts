...
export const appCenterEn = {
  ...
  localDev: {
    ...
    repairPrompt:
      'Adapt project directory "{{cwd}}" into a DinTalDock local debug app. Generate .tutti/dev-app/; bootstrap.sh must read the host-injected TUTTI_APP_HOST and TUTTI_APP_PORT, and must not hard-code a port.'
  },
  messages: {
    appInstallFailed:
...
export const appCenterZhCN = {
  ...
  localDev: {
    ...
    repairPrompt:
      "将项目目录“{{cwd}}”适配成本地调试 DinTalDock 应用。生成 .tutti/dev-app/；bootstrap.sh 必须读取宿主注入的 TUTTI_APP_HOST 和 TUTTI_APP_PORT，不要硬编码端口。"
  },
  messages: {
