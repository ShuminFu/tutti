...
description: "Build or evolve a complex agent-enabled DinTalDock workspace app repository. Use for DinTalDock apps with web/server/shared monorepos, @tutti-os/agent-acp-kit local agent runtimes, kit-owned TUTTI_CLI agent/composer discovery, dynamic agent catalogs, run-scoped MCP tool gateways, app-owned package builders, web-first debugging, i18n harnesses, and production package validation. For simple package creation or repair, use tutti-workspace-app-factory instead."
---

# DinTalDock Agent Workspace App

Use this skill when the task is to create or evolve a full DinTalDock app repository, not just a final package directory. The target app is usually a local-first web app with a server, shared contracts, optional local agent runtime, and an app-owned `scripts/package-tutti-app.mjs`.
...
- New agent-enabled DinTalDock app repositories.
- Existing web/server apps being converted into maintainable DinTalDock app repos.
...
- GitHub Actions release workflows for publishing DinTalDock app releases and staging/production catalogs.
...
- `references/package-builder.md` when adding `scripts/package-tutti-app.mjs`, `bootstrap.sh`, DinTalDock CLI output docs, or package validation.
...
5. If agents are needed, add an exact released `@tutti-os/agent-acp-kit`, use its default app-owned runtime, call the `/tutti` auto facade for catalog/composer/skill context, normalize events, and add a run-scoped tool gateway.
   ...
   For apps that must run both locally and in cloud/managed DinTalDock, follow `references/agent-acp-kit.md` exactly: managed credentials come from request headers on the server, never from browser JSB fallback or request body fields.
...
7. If the user asks to connect to the DinTalDock app ecosystem, treat ecosystem integration as required: expose app capabilities through `tutti.cli.json`, make the app callable by other DinTalDock apps and agents, and use `TUTTI_CLI` for any calls to other installed DinTalDock apps.
...
Keep DinTalDock-hosted and standalone behavior behind the kit's fixed auto facade while sharing the same runtime execution layer:
...
11. Package the built server, web assets, MCP/tool entrypoints, and runtime metadata needed by DinTalDock.
