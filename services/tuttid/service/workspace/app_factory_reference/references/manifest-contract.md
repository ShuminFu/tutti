# DinTalDock App Manifest Contract
...
- Do not include `runtime.kind`; DinTalDock manages the runtime baseline outside the app package.
...
- `cli` is optional. Include it only when the app exposes commands through the DinTalDock CLI.
...
- When `references.searchEndpoint` is present, DinTalDock marks the app as searchable and shows the picker search box for it. The endpoint must satisfy the same URL constraints as `listEndpoint`. Without it, the picker offers only the per-level `filterText` filtering of `listEndpoint`, never a global search.
- v1 references may only return groups and file references. File references must use `kind: "file"` and a `location` object. DinTalDock resolves the location to a filesystem path; apps must not emit host absolute paths.
...
- `filterText` is optional and already trimmed by DinTalDock. It filters only direct children of `parentGroupId`; it is not a recursive search.
- `limit` is clamped by DinTalDock to `1..50`.
- `cursor` is optional and opaque to DinTalDock.
...
- Apps must return `location`, not an absolute `path`. DinTalDock resolves valid locations to absolute paths before exposing results to desktop clients.
...
- Return results ordered by descending relevance and set `score` (`0..1`) so DinTalDock can preserve your ranking. When `score` is omitted DinTalDock keeps the returned order.
...
- `parentGroupLabel` is optional (string, max 160 chars). Because search is flattened across the whole app, set it to the name of the group/project the file lives in (e.g. the project a design belongs to) so users can tell results apart. DinTalDock shows each item's `displayName` as the title and `parentGroupLabel` as the context subtitle. When you omit it, DinTalDock falls back to your app's manifest display name as the subtitle, so still keep the manifest display name and each `displayName` meaningful.
