...
export function createDesktopPreferencesMutation(
  ...
  return {
    ...
    // Unrelated full-preference writes omit these columns. The daemon keeps
    // their latest stored values atomically when another settings surface edits
    // retention concurrently.
    ...(overrides.agentRuntimeKeepAliveEnabled !== undefined
      ? { agentRuntimeKeepAliveEnabled: overrides.agentRuntimeKeepAliveEnabled }
      : {}),
    ...(overrides.agentRuntimeIdleMinutes !== undefined
      ? { agentRuntimeIdleMinutes: overrides.agentRuntimeIdleMinutes }
      : {}),
    ...(overrides.agentRuntimeMaxResident !== undefined
      ? { agentRuntimeMaxResident: overrides.agentRuntimeMaxResident }
      : {}),
